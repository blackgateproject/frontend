...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (walletExists) {
      try {
        ...
        setWallet(wallet);
        setSigner(signer);
        setIsWalletUnlocked(true); // Update state when wallet is unlocked
      } catch (error) {
  ...
  return (
    <div className="h-screen flex items-center justify-center bg-gradient-to-br from-gray-400 to-black">
      <div className="bg-base-100 p-10 rounded-2xl shadow-xl w-96 overflow-hidden">
        <div className="flex justify-center items-center mb-4">
          <img
            src={logo}
            alt="logo"
            className="w-24 transition-transform hover:scale-105 duration-300"
          />
        </div>
        <h2 className="text-center text-3xl font-bold text-primary">
          ...
          {isWalletUnlocked && (
            <p className="text-sm text-green-500">Wallet Unlocked</p>
          )}
        </h2>
      ...
      <dialog
        id="signup-modal"
        className="modal modal-bottom sm:modal-middle backdrop-brightness-75"
